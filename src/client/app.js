...
function getColorDark(givenColor) {
	const dark = config.graphical.neon ? color.white : color.black;
...
function getEntityImageFromMockup(index, color = mockups[index].color) {
	...
	return {
		...
		turrets: mockup.turrets.map((t) => {
			const o = getEntityImageFromMockup(t.index);
	...
	const Region = (() => {
		...
		function Clickable() {
			...
			return {
				...
				check: (target) => {
					const dx = Math.round(target.x - region.x);
					const dy = Math.round(target.y - region.y);
					return (
						active &&
						dx >= 0 &&
						dy >= 0 &&
	...
	return {
		get: () => {
			...
			for (const e in entries) {
				if (!Object.hasOwn(entries, e)) continue;
				const data = entries[e].publish();
				out.push(data);
		...
		add: (data) => {
			const newentry = Entry(data.name, data.barcolor, data.color);
...
var moveCompensation = (() => {
	...
	return {
		...
		iterate: (g) => {
			...
			// Add motion
			const damp = gui.accel / gui.topSpeed,
				len = Math.sqrt(g.x * g.x + g.y * g.y);
			vx += (gui.accel * g.x) / len;
			vy += (gui.accel * g.y) / len;
			// Dampen motion
			const motion = Math.sqrt(vx * vx + vy * vy);
			if (motion > 0 && damp) {
				const finalvelocity = motion / (damp / roomSpeed + 1);
					...
					return () => {
						...
						switch (type) {
							case -1:
								{
									// removal
									const index = minimap.findIndex((e) =>
...
function startGame() {
	...
	const a = document.getElementById("optColors").value;
...
const TextObj = (() => {
	...
	return () => {
		...
		const canvasId = "textCanvasNo" + index++;
		...
		const vals = floppies.map((f) => f.publish());
		let xx = 0;
		let yy = 0;
		return {
			draw: (
				...
				if (floppies.some((f) => f.check())) {
					...
					const offset = Math.max(3, size / 5);
					const dim = measureText(
						text,
						size - config.graphical.fontSizeBoost,
						true,
					);
...
const drawEntity = (() => {
	...
	function drawPoly(
		...
		if (!sides) {
			...
			for (let i = 0; i < sides; i++) {
				...
				const y = centerY + radius * Math.sin(theta + angle);
				context.lineTo(x, y);
			}
		}
		context.closePath();
	...
	function drawTrapezoid(context, x, y, length, height, aspect, angle) {
		...
		const r = [Math.atan2(h[0], length), Math.atan2(h[1], length)];
		const l = [
...
function drawHealth(x, y, instance, ratio) {
	// Draw health bar
	ctx.globalAlpha = instance.render.status.getFade() ** 2;
	const size = instance.size * ratio;
	const m = mockups[instance.index];
	const realSize = (size / m.size) * m.realSize;
	// Draw health
	if (instance.drawsHealth) {
		const health = instance.render.health.get();
		const shield = instance.render.shield.get();
		if (health < 1 || shield < 1) {
			const yy = y + 1.1 * realSize + 15;
			drawBar(
				x - size,
				x + size,
				yy,
...
const gameDraw = (() => {
	...
	return (ratio) => {
		...
		let px, py;
		{
			...
			motion.set();
			const smear = { x: 0, y: 0 }; // moveCompensation.get();
...

		{
			...
			let ticker = 11;
			const namedata = gui.getStatNames(
				mockups[gui.type].statnames || -1,
			);
			gui.skills.forEach(function drawASkillBar(skill) {
				// Individual skill bars
				ticker--;
				const name = namedata[ticker - 1],
					level = skill.amount,
...

		{
			...
			const vspacing = 4;
			const len = 1.65 * alcoveSize * global.screenWidth;
			let height = 25;
			const x = (global.screenWidth - len) / 2;
			let y = global.screenHeight - spacing - height;
...

		{
			...
			const len = alcoveSize * global.screenWidth;
			const height = len;
			const x = global.screenWidth - len - spacing;
			const y = global.screenHeight - height - spacing;
...

		{
			...
			lb.data.forEach((entry) => {
				...
				drawBar(x, x + len, y + height / 2, height - 3, color.grey);
				const shift = Math.min(1, entry.score / max);
				drawBar(
					x,
					x + len * shift,
					y + height / 2,
				...
				// Mini-image
				const scale = height / entry.position.axis,
					xx =
						x -
						1.5 * height -
...

		{
			...
			const glide = upgradeMenu.get();
			global.clickables.upgrade.hide();
			if (gui.upgrades.length > 0) {
				global.canUpgrade = true;
				var getClassUpgradeKey = (number) => {
					switch (number) {
						case 0:
							return "y";
						case 1:
				...
				const len = ((alcoveSize * global.screenWidth) / 2) * 1;
				const height = len;
				...
				gui.upgrades.forEach(function drawAnUpgrade(model) {
					...
					const picture = getEntityImageFromMockup(model, gui.color),
				...
				const xx = xo + (xxx + len + internalSpacing - xo) / 2,
...
const gameDrawDead = (() => {
	...
	const getKills = () => {
		...
		const b = finalKills[0] + 0.5 * finalKills[1] + 3 * finalKills[2];
		return (
			(b === 0
				? "🌼"
				: b < 4
	...
	return () => {
		...
		const picture = getEntityImageFromMockup(gui.type, gui.color),
			len = 140,
