...
exports.handleLargeNumber = (a, cullZeroes = false) => {
	...
	if (a < 10 ** 6) {
		return (a / 10 ** 3).toFixed(2) + "k";
	}

	if (a < 10 ** 9) {
		return (a / 10 ** 6).toFixed(2) + "m";
	}

	if (a < 10 ** 12) {
		return (a / 10 ** 9).toFixed(2) + "b";
	}

	if (a < 10 ** 15) {
		return (a / 10 ** 12).toFixed(2) + "t";
	}

	return (a / 10 ** 15).toFixed(2) + "q";
};
...
exports.pullJSON = (filename) => {
	...
	const url = "/json/" + filename + ".json?v=" + VERSION;
	// Set up the request
