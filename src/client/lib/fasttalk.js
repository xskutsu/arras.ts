...
exports.encode = (() => {
	...
	return (arr, verbose = false) => {
		...
		const integerView = new Uint8Array(buffer);
		for (let i = 0; i < len; i++) {
...
exports.decode = (() => {
	...
	return (raw) => {
		try {
			const intView = new Uint8Array(raw);
			let str = "";
			...
			while (offset < str.length) {
				switch (str[offset++]) {
					...
					case "7":
						{
							// String8
							const len = typeDecoder(str, "Uint16", offset);
							offset += 2;
					...
					case "8":
						{
							...
							const len = typeDecoder(str, "Uint16", offset);
							offset += 2;
							const arr = str.slice(offset, offset + len);
							const buf = new Uint16Array(len / 2);
					...
					default:
						offset = str.length;
						throw new Error(
							"Unknown decoding command. Decoding exited.",
						);
				}
			}
			return output;
